...
# STEP 3: Reprojection Error as RE
	...
	RU_PercentageRemove = float(sys.argv[1] if paramNo == 10 else def_RU_PercentageRemove)
	RU_ThreshMax = float(sys.argv[2] if paramNo == 10 else def_RU_ThreshMax)
	RU_Value = float(sys.argv[3] if paramNo == 10 else def_RU_Value)
	
	PA_PercentageRemove = float(sys.argv[4] if paramNo == 10 else def_PA_PercentageRemove)
	PA_ThreshMax = float(sys.argv[4] if paramNo == 10 else def_PA_ThreshMax)
	PA_Value = float(sys.argv[4] if paramNo == 10 else def_PA_Value)
	
	RE_PercentageRemove = float(sys.argv[4] if paramNo == 10 else def_RE_PercentageRemove)
	RE_MaxIterations = float(sys.argv[4] if paramNo == 10 else def_RE_MaxIterations)
	RE_Value = float(sys.argv[4] if paramNo == 10 else def_RE_Value)
	
	perc_total_thresh = float(sys.argv[4] if paramNo == 10 else def_perc_total_thresh)
...
# RU loop
	...
	while RU_refined == False:
		...
		f = Metashape.TiePoints.Filter()                             # initialise cloud filter based on criteria
		f.init(pc, criterion=Metashape.TiePoints.Filter.ReconstructionUncertainty) 
...
## PA loop
	...
	while PA_refined == False:
		...
		f.init(pc, criterion=Metashape.TiePoints.Filter.ProjectionAccuracy)
		values = f.values.copy()
		values.sort()
...
#  Note : important to verify that cameras have enough projections at the end (in reference pane), if needed modify parameter block to limit iteration or total points removed
	...
	while RE_refined == False:
		...
		f = Metashape.TiePoints.Filter()
		f.init(pc, criterion=Metashape.TiePoints.Filter.ReprojectionError)
		values = f.values.copy()
		values.sort()
